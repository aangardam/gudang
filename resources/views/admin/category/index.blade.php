...
<script>
  window.remove = function (id) {
      ...
      swal({
          ...
          text: "Kategori yang sudah di hapus tidak dapat di kembalikan!",
          type: "warning",
  ...
  window.inactive = function (id) {
      ...
      swal({
          title: "Apakah Anda Yakin?",
          text: "Kategori akan di Non Activkan",
          type: "warning",
          showCancelButton: true,
          confirmButtonColor: "#DD6B55",
  ...
  window.active = function (id) {
      ...
      swal({
          title: "Apakah Anda Yakin?",
          text: "Kategori akan di Activkan kembali",
          type: "warning",
          showCancelButton: true,
          confirmButtonColor: "#DD6B55",
      ...
      <div class="tile">
        <div align="right">
            <a href="{{ url ('Kategori/create') }}" class="btn btn-primary btn-sm">
                <b><i class="fa fa-plus"></i></b> Kategori
            </a>
        </div>
        <hr>
              ...
              <tr>
                ...
                <td>
                    <div class="toggle">
                      ...
                      <form id="inactive-{{$value->id}}" action="{{ url('/Kategori/inactive/'.$value->id ) }}" method="POST"style="display: none;">
                        {{ csrf_field() }}
                      </form> 
                        <a href="#" onclick="inactive({{$value->id}})"><input type="checkbox" checked=""><span class="button-indecator"></span></a>
                      @else
                      <form id="active-{{$value->id}}" action="{{ url('/Kategori/active/'.$value->id ) }}" method="POST"style="display: none;">
                        {{ csrf_field() }}
                      </form> 
                ...
                <td>
                  @if ($value->status != 'Aktif')
                    <form id="delete-{{$value->id}}" action="{{ action('CategoryController@destroy', ['id' => $value->id]) }}" method="POST" style="display: none;">
                      {{ csrf_field() }}
                      {{ method_field('DELETE') }}
                    </form>
                  ...
                  <a href=" {{  url('Kategori/'.$value->id.'/edit') }} " class="btn btn-primary btn-xs" style="padding: 0px 5px 0px 5px"> Ubah </a>
                </td>
